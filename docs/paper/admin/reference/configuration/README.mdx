...
我们的许多文件都经过了全面的文档记录。 如果是这种情况，它们将提供指向其页面的链接。 如果不是这种情况，它们会简要解释它们是什么。
...
Paper 配置最强大但最不被理解的功能之一是**按世界**设置配置选项。 虽然您无法按世界覆盖每个配置选项，但 `paper-world-defaults.yml` 中存储的所有内容都可以。

### 默认值

Paper 在出厂设置中没有设置任何按世界覆盖，而是将所有默认值存储在 `config/paper-world-defaults.yml` 中。 此文件中的所有内容都可以按世界覆盖，但默认情况下不会覆盖。 更改 `paper-world-defaults.yml` 中的某些内容将更改您**未**手动覆盖它的**所有世界**的值。
...
默认情况下，`paper-world.yml` 配置文件中仅设置了 `_version`。 为了覆盖选项的默认值，您必须从 `paper-world-defaults.yml` 手动添加它。

### 继承

所有未明确为世界定义的配置都从 `paper-world-defaults.yml` 继承。 这意味着无需在 `paper-world-defaults.yml` 和每个单独的 `paper-world.yml` 之间重复自己。 您**不需要也不应该**将整个 `paper-world-default.yml` 文件复制到您想要修改的每个 `paper-world.yml` 文件中。 仅复制您想要更改的确切值。

对于更复杂的真实世界示例：在两个世界中设置不同的 `spawn-limits` 和 `auto-replenish`。
